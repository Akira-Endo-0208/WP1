...
public class SearchCharacter : MonoBehaviour {
    ...
    void OnTriggerStay(Collider col) {
        ...
        if (col.tag == "Player") {
            ...
            {
                if ((Vector3.Distance(col.transform.position, transform.position) <= searchArea.radius * 0.5f) && !Physics.Linecast(transform.position + Vector3.up, col.transform.position + Vector3.up, obstacleLayer))
                {
                    moveEnemy.SetState(MoveEnemy.EnemyState.Chase, col.transform);
                }
            }
